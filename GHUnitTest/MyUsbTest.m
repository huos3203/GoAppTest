...
@interface MyUsbTest : GHTestCase<PTChannelDelegate>

@end

@implementation MyUsbTest
{
...
-(void)createNewChannel
{
    ...
    [channel listenOnPort:PTExampleProtocolIPv4PortNumber IPv4Address:INADDR_LOOPBACK callback:^(NSError *error) {
        if (error) {
            GHTestLog(@"Failed to listen on 127.0.0.1:%d: %@", PTExampleProtocolIPv4PortNumber, error);
        } else {
            GHTestLog(@"Listening on 127.0.0.1:%d", PTExampleProtocolIPv4PortNumber);
...
- (void)ioFrameChannel:(PTChannel*)channel didReceiveFrameOfType:(uint32_t)type tag:(uint32_t)tag payload:(PTData*)payload {
    ...
    if (type == PTExampleFrameTypeTextMessage) {
        ...
        GHTestLog(@"[%@]: %@", channel.userInfo, message);
...
- (void)ioFrameChannel:(PTChannel*)channel didEndWithError:(NSError*)error {
    if (error) {
        GHTestLog(@"%@ ended with error: %@", channel, error);
        
    } else {
        GHTestLog(@"Disconnected from %@", channel.userInfo);
    }
}
...
- (void)ioFrameChannel:(PTChannel*)channel didAcceptConnection:(PTChannel*)otherChannel fromAddress:(PTAddress*)address {
    ...
    GHTestLog(@"Connected to %@", address);
    // 把关于自己的一些信息发送到另一端
    //    [self sendDeviceInfo];
}
