...
+(NSMutableDictionary *)readFilePlilst:(NSString *)fileName
{
    ...
    NSMutableDictionary *nsmuDict=[[NSMutableDictionary alloc]initWithContentsOfFile:path];
    if (nsmuDict) {
        return  nsmuDict;
    }
    return nil;
...
+(NSString *)getFilePathOfFileExists:(NSString *)fileName
{
    ...
    BOOL isDirectory = NO;
	BOOL fileExists = [[[NSFileManager alloc] init] fileExistsAtPath:fullPathToFile isDirectory:&isDirectory];
